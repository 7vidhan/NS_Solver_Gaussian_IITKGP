...
double T[1000000], Told[1000000], s_t[1000000],domain_radius;
...
double solve() {
   ...
   er_mx1 = 0.;
	...
	for (i = 1; i <= nx; i++) {
		for (j = 1; j <= ny; j++) {
		   ij  = i-1 + (j-1)*nx;
         T[neighbor[ij][0]] = (s_t[neighbor[ij][0]]- a_coeff[ij][1]*Told[neighbor[ij][1]] - a_coeff[ij][2]*Told[neighbor[ij][2]] - a_coeff[ij][3]*Told[neighbor[ij][3]] - a_coeff[ij][4]*Told[neighbor[ij][4]] - a_coeff[ij][5]*Told[neighbor[ij][5]])/ a_coeff[ij][0];
			//printf("%d %d %d %lf %lf\n", i, j, ij, T[neighbor[ij][0]], Told[neighbor[ij][0]]);
...
void neighbor_points() {
	int i, j, ij, ij1,m,counting,n;
	counting = (nx+2)*(ny+2);
	int locationarray[counting];
	//int* storeit;
	double radist[counting];
	double *storedist;
	n = 1; domain_radius = 0.2;
	...
	for (j = 1; j<=ny; j++) {
		for (i = 1; i<=nx; i++){
		   ...
		   for (m=0; m<counting; m++){
			...
			printf ("%d %d, %d %d %d %d %d\n", ij1, ij, neighbor[ij][0],neighbor[ij][1],neighbor[ij][2],neighbor[ij][3],neighbor[ij][4]);
                }
