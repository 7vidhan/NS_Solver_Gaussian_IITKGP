...
int check(double random1,double random2, int i, int j){
    int a,b,flag,ab;
    for (b=0;b<j;b++){
            for (a=0;a<i;a++){
                ab = a + b*(nx+2);
               if (((random1/9999)*lx == x[ab]) && ((random1/9999)*ly == y[ab]) ) {flag=1;}
               if (((random2/9999)*lx == x[ab]) && ((random2/9999)*ly == y[ab]) && flag==1 ) {flag += 1; return flag;}
      }
    }
    return 0;
}
...
void read_point_data() {
   ...
   for (j = 0; j <= ny+1; j++) {
		for (i = 0; i <= nx+1; i++) {
            double random1, random2; int test=0;
            random1 = rand()%10000;
            random2 = rand()%10000;
            test = check(random1,random2,i,j);
            while (test != 0){
                random1 = rand()%10000;
                random2 = rand()%10000;
                test = check(random1,random2,i,j);
             ...
             ij = i + j*(nx+2);
         x[ij] = (random1/9999)*lx;
         y[ij] = (random2/9999)*ly;
...
void d2fd2(int ij){
	...
	d2phid2[0] = exp(-pow((x[neighbor[ij][2]] - x[neighbor[ij][0]])/sigma,2))*exp(-pow((y[neighbor[ij][2]] - y[neighbor[ij][0]])/sigma,2))*(-4/pow(sigma,2) + 4*pow(x[neighbor[ij][2]] - x[neighbor[ij][0]],2)/pow(sigma,4) + 4*pow(y[neighbor[ij][2]] - y[neighbor[ij][0]],2)/pow(sigma,4));

	d2phid2[1] = exp(-pow((x[neighbor[ij][2]] - x[neighbor[ij][1]])/sigma,2))*exp(-pow((y[neighbor[ij][2]] - y[neighbor[ij][1]])/sigma,2))*(-4/pow(sigma,2) + 4*pow(x[neighbor[ij][2]] - x[neighbor[ij][1]],2)/pow(sigma,4) + 4*pow(y[neighbor[ij][2]] - y[neighbor[ij][1]],2)/pow(sigma,4));

	d2phid2[2] = exp(-pow((x[neighbor[ij][2]] - x[neighbor[ij][2]])/sigma,2))*exp(-pow((y[neighbor[ij][2]] - y[neighbor[ij][2]])/sigma,2))*(-4/pow(sigma,2) + 4*pow(x[neighbor[ij][2]] - x[neighbor[ij][2]],2)/pow(sigma,4) + 4*pow(y[neighbor[ij][2]] - y[neighbor[ij][2]],2)/pow(sigma,4));

	d2phid2[3] = exp(-pow((x[neighbor[ij][2]] - x[neighbor[ij][3]])/sigma,2))*exp(-pow((y[neighbor[ij][2]] - y[neighbor[ij][3]])/sigma,2))*(-4/pow(sigma,2) + 4*pow(x[neighbor[ij][2]] - x[neighbor[ij][3]],2)/pow(sigma,4) + 4*pow(y[neighbor[ij][2]] - y[neighbor[ij][3]],2)/pow(sigma,4));

	d2phid2[4] = exp(-pow((x[neighbor[ij][2]] - x[neighbor[ij][4]])/sigma,2))*exp(-pow((y[neighbor[ij][2]] - y[neighbor[ij][4]])/sigma,2))*(-4/pow(sigma,2) + 4*pow(x[neighbor[ij][2]] - x[neighbor[ij][4]],2)/pow(sigma,4) + 4*pow(y[neighbor[ij][2]] - y[neighbor[ij][4]],2)/pow(sigma,4));
